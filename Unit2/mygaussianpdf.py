...
class MyGaussianPDF():
    ...
    def __init__(self,mean,stdev,a,b):
        self.mean = mean
        self.stdev = stdev
        self.a=a
        self.b=b
    ...
    def eval(self, xval):
        result = (1.0/(self.stdev*(math.sqrt(2*math.pi))))*np.exp(-1.0*(((xval - self.mean)/self.stdev)**2)/2.0)
        return result

        #evaluating gaussian at mean is maximum
    def max(self):
        val = self.eval(self.mean)
        return val
    ...
    def analyticint(self,low,high):
        answer = self.stdev*math.sqrt(2.0*math.pi)
        return answer
