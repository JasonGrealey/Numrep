...
class ODErealefield():
    ...
    def firstderiv(self,coords):
        if 0<=coords[0]<=4:
            return(  np.piecewise(coords[0], [0<=coords[0] <= 0.99, 0.99<= coords[0] <=1.99,1.99<=coords[0]<=2.99,2.99 <= coords[0] <= 3.99], [0.,self.height,(-1.0) *(self.height),0.]))
        else:
            return (0.)
...
class perform:
    ...
    def run(self,nsteps,dx):
        ...
        for i in range (nsteps):
            ...
            deltal = self.step.dy(self.ode, now, dx)
            new = [now[0]+deltal[0], now[1]+deltal[1]]
            res.app(new)
